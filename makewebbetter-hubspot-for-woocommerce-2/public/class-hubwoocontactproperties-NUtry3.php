 ...
 *
 * @since 1.0.0
 * @param int $last_order_id last order id to create html.
 */
	...
	if ( ! empty( $last_order_id ) ) {
		...
		if ( ! empty( $order ) && ! is_wp_error( $order ) ) {
			...
			if ( is_array( $order_items ) && count( $order_items ) ) {
				...
				foreach ( $order_items as $item_id_1 => $wc_order_item_product ) {
					...
					if ( ! empty( $wc_order_item_product ) && $wc_order_item_product instanceof WC_Order_Item ) {
						...
						$product_id = $wc_order_item_product->get_variation_id();
						if ( empty( $product_id ) ) {
			...
			if ( count( $last_order_products ) ) {
				$products_html = '<div><hr></div><!--[if mso]><center><table width="100%" style="width:600px;"><![endif]--><table style="font-size: 14px; font-family: Arial, sans-serif; line-height: 20px; text-align: left; table-layout: fixed;" width="100%"><thead><tr><th style="text-align: center; word-wrap: unset;">' . __( 'Image', 'makewebbetter-hubspot-for-woocommerce' ) . '</th><th style="text-align: center; word-wrap: unset;">' . __( 'Item', 'makewebbetter-hubspot-for-woocommerce' ) . '</th></tr></thead><tbody>';

				foreach ( $last_order_products as $product ) {
					$products_html .= '<tr><td style="max-width: 20%; text-align: center;"><img height="50" width="50" src="' . esc_url( $product['image'] ) . '"></td><td style="max-width: 80%; text-align: center; font-size: 12px;"><a target="_blank" href="' . esc_url( $product['url'] ) . '"><strong>' . esc_html( $product['name'] ) . '</strong></a></td></tr>';
				}
